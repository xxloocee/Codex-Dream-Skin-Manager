...
- 新增每套主题独立保存的「消息气泡不透明度（用户与助手）」参数，范围 0% 至 100%；新建主题和“已存主题设置”均可调整。当前活动主题保存后立即刷新，无需重启 Codex。
- 修复任务页顶部栏重新出现深色不透明遮罩的问题；任务页标题栏、原生应用菜单栏与窗口顶部区域现在均透出背景，不再叠加边框、阴影或毛玻璃。
- 用户消息和助手回复统一读取同一个气泡不透明度参数；默认 0% 保持完全透明，兼容流式回复、最终回复和新版用户气泡节点。
- 自定义换肤页改为“参数区域可滚动、保存按钮固定在底部”的布局，新增参数后在小窗口中仍可直接保存或保存并应用。
- 新参数已同步至 Windows、macOS 注入器、主题包校验器和运行时资源；主题包导入、导出、批量导入与已有主题编辑均可保留该值。

## 1.7.4 — 2026-09-18

- 修复新版 Codex 中用户消息、助手回复和“运行了命令”等工具结果仍显示深色不透明气泡的问题。
- 样式改为直接匹配 Codex 当前稳定的 `data-user-message-bubble` 与 `data-response-annotation-*` 节点，同时保留旧版对话锚点兼容；三类会话容器现在均无背景、边框、阴影和毛玻璃。
- 将该修复写入跨平台唯一的 `runtime/dream-skin.css` 并重新同步 Windows、macOS 运行时资源，避免后续构建或合并时覆盖 Windows 修复。
- 补回 `macos/client-delivery` 下两个被旧目录复制遗漏、但由 Git 跟踪的客户交付文档；源码工作树恢复为无已跟踪文件缺失状态。

## 1.7.3 — 2026-09-18

- 控制台现在支持删除未使用的内置主题；删除后主题条目从内置主题库移除，背景媒体移入 Windows 回收站。
- 保护当前正在使用的主题与 `paper-light` 默认恢复主题，避免误删导致当前皮肤或“重置皮肤”不可用。
- 管理接口新增 `DeletePreset`，与原有“我的”主题删除共用明确的选中、活动状态与确认提示。
- 程序窗口、程序集标题、单实例唤醒标题和桌面快捷方式统一命名为 `Codex Dream Skin Manager`。

## 1.7.2 — 2026-09-18

- 将“已存主题参数编辑器”完整合并进 Windows 主程序，新增“已存主题设置”页面；不再需要第二个 PowerShell 入口。
- 控制台中选中“我的”主题时新增“编辑主题参数”按钮，可直接跳转到该主题的主程序内编辑页。
- 主程序内可修改外观模式、焦点、文字安全区、任务页模式、强调色和自定义取景；修改当前活动主题时立即刷新，不重启 Codex。
- 管理器窗口标题、程序集标题和说明改为中文；保留内部兼容参数值与原有主题文件格式。

## 1.7.1 — 2026-09-18

- 修复宽幅背景在「任务页模式 = 完整」时，普通任务页顶部栏仍被 90% 不透明面板遮挡的问题。完整模式现在与正文保持连续背景，同时保留原生窗口顶栏、首页、侧栏和输入框的既有规则。
- 新增已存主题参数编辑器，可直接编辑已有主题的外观、取景、文字安全区、任务页模式和强调色；修改当前活动主题时实时重载，不重新创建主题。
- 修复参数编辑器的主题名称绑定和下拉控件颜色：保存主题按名称显示，弹出菜单及关闭状态均采用深色底与浅色字。
- 参数编辑器的选项全部中文化，同时保留主题文件所需的兼容参数值。
- 左侧栏的待关注提醒、未读圆点及更新徽标现在跟随主题强调色。
- 参数编辑器由隐藏控制台启动，避免打开或停留可见的 PowerShell 窗口。
- 修复新版 Codex 用户消息内层气泡未跟随透明会话背景的问题。
- 优化已存主题保存/应用：删除切换时冗余的归档副本、全库归档哈希与重复元数据解析；MP4 解码验证及启动安全验证保持不变。
- 缩短首次受管启动时 CDP 检测和渲染器验证重试间隔，保留原有验证截止时间与失败回滚边界。
