...
- 图片和 MP4 导入上限统一提高到 128 MiB，ZIP 包上限调整为 160 MiB、解压总量上限为 192 MiB。
- 图片改用文件快照及本地 blob 加载，与视频共用有界缓存，避免 Base64 注入。
- 支持标准非分片 H.265/HEVC MP4；导入、应用和恢复动态皮肤前必须通过 Codex 实际视频帧解码校验。未连接时提示先启动并连接，失败时保留当前皮肤。
- Windows 主题库整理为八大分类，支持多选标签，优化顶部布局和控件，并新增捐赠入口。

## 1.6.1 — 2026-09-17

- 新增「墨羽流光」「银璃幻梦」两套 MP4 动态内置主题，以及「灯下闲居」高清静态内置主题，保留原始媒体和强调色。
- 内置目录增加至 36 套；新增主题跟随 Codex 浅色或深色外观，支持分类及标签筛选。

## 1.6.0 — 2026-09-16

### 新增

- 33 套内置主题全部改为跟随 Codex 当前的浅色/深色外观；主题原有壁纸、构图、强调色与特殊样式继续保留，自定义和导入主题仍可显式指定浅色或深色。
- 背景媒体支持 GIF、APNG 和 MP4；管理器可直接预览视频主题，并通过本地文件桥接加载 MP4，不把视频内容转成 Base64。
- Windows Release 同时提供 x64 安装版与便携版，并与 macOS Universal DMG 一起发布统一的 SHA-256 校验清单。
- 内置更新检查改为查询本项目的 GitHub Releases，后续管理器版本可被正确发现。

### 修复

- “暖光凝眸”和“日光窗影”恢复为 2560×1440 的原始高清壁纸，不再使用 119×76 的缩略图资源。
- 已应用的旧版内置主题会在预设 ID 与图片 SHA-256 同时匹配时安全迁移为自适应外观；用户自行修改或导入的主题不会被覆盖。
