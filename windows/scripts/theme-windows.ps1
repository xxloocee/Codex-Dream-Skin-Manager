...
function Initialize-DreamSkinThemeStore {
  ...
  # Official presets are read-only packages under the skill root. They must
  # not be copied into themes/, where the manager would expose them as user
  # themes that can be deleted. Remove only old staged copies whose package
  # identity and artwork still match the bundled source exactly.
  $officialPresetSources = @(
    [pscustomobject]@{
      Id = $bundledPresetId
      Directory = $assetRoot
      Image = $assetImage
  ...
  if ((Test-Path -LiteralPath $gothicSourceTheme -PathType Leaf) -and
    ...
    $officialPresetSources += [pscustomobject]@{
      Id = 'preset-gothic-void-crusade'
      Directory = $gothicSource
      Image = $gothicSourceImage
    }
  }
  foreach ($official in $officialPresetSources) {
    $stagedDirectory = Join-Path $paths.Saved $official.Id
    if (-not (Test-Path -LiteralPath $stagedDirectory -PathType Container)) { continue }
    try {
      Assert-DreamSkinNoReparseComponents -Path $stagedDirectory
      $staged = Read-DreamSkinTheme -ThemeDirectory $stagedDirectory -SkipImageMetadata
      $source = Read-DreamSkinTheme -ThemeDirectory $official.Directory -SkipImageMetadata
      if ("$($staged.Theme.id)" -cne "$($official.Id)" -or
        "$(if ($staged.Theme.name) { $staged.Theme.name } else { '' })" -cne
          "$(if ($source.Theme.name) { $source.Theme.name } else { '' })") { continue }
      $stagedImageHash = (Get-FileHash -LiteralPath $staged.ImagePath -Algorithm SHA256).Hash
      $sourceImageHash = (Get-FileHash -LiteralPath $source.ImagePath -Algorithm SHA256).Hash
      if ($stagedImageHash -cne $sourceImageHash) { continue }
      Remove-Item -LiteralPath $stagedDirectory -Recurse -Force -ErrorAction Stop
    } catch {
      # A user-owned or otherwise unverifiable directory is left untouched.
    }
  }
