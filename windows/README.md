...
- 更换 PNG、APNG、JPEG、WebP、GIF 背景图或标准非分片 H.264/AVC 或 H.265/HEVC MP4 视频；动画图片和 MP4 都保留原格式。MP4 验证后会生成只读的内容寻址快照，再通过受控文件桥创建本地 `blob:` 地址，不会编码为 Base64，并在 Codex 中静音循环播放。托盘管理器通过 Windows 系统媒体缩略图显示 MP4 静态封面，但不播放视频。
...
导入背景必须是纯素材，不要使用包含窗口、侧栏、输入框、文字或按钮的效果截图。图片上限为 128 MiB，MP4 上限为 128 MiB；宽或高不能超过 16384 像素，总像素不能超过 5000 万；动图最多 300 帧。MP4 必须是标准非分片 H.264/AVC 或 H.265/HEVC 文件，最长 60 秒、最高 60 FPS；原文件直接保存，不会转码。

新的正式 Studio ZIP 必须包含 `manifest.json`、非空 `theme.json`、非空 `theme.css`、恰好一个 `background.webp|jpg|png|apng|gif|mp4`，并可选
带 `LICENSE.txt`、`manifest.sig`；文件直接位于根目录或只包一层主题目录。本地简化包也必须恰好包含
`theme.json`、`theme.css` 与其引用图片，且只应来自可信来源。压缩文件上限 160 MiB、最多
32 个条目、解压后最多 192 MiB；路径穿越、链接/reparse、嵌套压缩包和未注册文件会被拒绝。正式包还会
...
MP4 导入或应用前必须连接 Codex 并通过真实视频帧解码校验。无法连接、解码失败或超时会阻止操作；支持 H.265 不代表每台设备都能播放，不自动转码。
