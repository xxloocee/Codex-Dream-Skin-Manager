...
((cssText, artDataUrl, themeConfig) => {
  ...
  const THEME_VARIABLES = [
    ...
    "--dream-art-framing-position", "--dream-art-background-size", "--dream-art-zoom",
    "--dream-art-position-x", "--dream-art-position-y",
  ...
  const applyArtMetadata = (root) => {
    ...
    // Framing is intentionally expressed as CSS variables rather than DOM
    // measurements. The same payload therefore works in the Windows and
    // macOS renderers, while legacy themes continue to use their focus data.
    ...
    const positionX = typeof ART.positionX === "number" ? clamp(ART.positionX, -1, 1) : 0;
    const positionY = typeof ART.positionY === "number" ? clamp(ART.positionY, -1, 1) : 0;
    const zoom = typeof ART.zoom === "number" ? clamp(ART.zoom, 1, 2) : 1;
    const positionMode = ART.positionMode === "free" ? "free" : "locked";
    const movementScale = positionMode === "free" ? 100 : 50;
    const framingPosition = `${(50 + positionX * movementScale).toFixed(2)}% ` +
      `${(50 + positionY * movementScale).toFixed(2)}%`;
    const backgroundSize = zoom === 1 ? "cover" : `${(zoom * 100).toFixed(2)}% auto`;
    ...
    setStyleProperty(root, "--dream-art-position-x", String(Number(positionX.toFixed(4))));
    setStyleProperty(root, "--dream-art-position-y", String(Number(positionY.toFixed(4))));
