...
当前版本：`1.7.8`。运行时支持 Windows 和 macOS；WPF 管理器仍是 Windows 专用界面，macOS 提供原生菜单栏界面及一个共享状态/framing 契约的 `manager-actions-macos.sh` 动作适配层。

## 下载

正式版本从 [GitHub Releases](https://github.com/xxloocee/Codex-Dream-Skin-Manager/releases) 下载：

- `CodexDreamSkinManager-v1.7.8-windows-x64-setup.exe`：Windows 10/11 x64 推荐安装版。
- `CodexDreamSkinManager-v1.7.8-windows-x64-portable.zip`：Windows 10/11 x64 便携版。
- `CodexDreamSkinManager-v1.7.8-macos-x64.dmg`：macOS 13+，Intel Mac 安装包。
- `CodexDreamSkinManager-v1.7.8-macos-arm64.dmg`：macOS 13+，Apple Silicon Mac 安装包。
...
安装包会生成到 `dist\CodexDreamSkinManager-v1.7.8-windows-x64-setup.exe`。Inno Setup 只用于构建，
不会成为用户电脑上的运行依赖。
