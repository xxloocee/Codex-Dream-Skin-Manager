...
- 原生菜单栏客户端现在随 GitHub Release 提供同时支持 Apple Silicon 与 Intel 的通用 DMG，版本号、运行时与发布 Tag 统一。
- 背景媒体支持 GIF、APNG 和 MP4；MP4 使用本地文件桥接加载，并在管理器中生成受限尺寸的预览缩略图。
- 内置 Gothic 主题改为跟随 Codex 当前的浅色/深色外观，同时保留它自身的构图、强调色和特殊样式。

### 修复

- 内置主题元数据更新后，macOS 应用包的审核哈希同步更新，继续阻止未经复核的公开预设进入发布包。

### 发布

- 发布流程现在必须同时完成 Windows x64 和 macOS Universal 构建，随后才创建 Release，并为全部安装包生成统一的 `SHA256SUMS.txt`。
- 应用内更新检查与“查看最新版本”入口改为本项目的 GitHub Releases，不再跳转到版本线不同的上游发布页。
...
- 菜单栏新增 System / English / 中文语言选择，选择会持久保存，并覆盖状态、主题操作、更新、恢复/卸载和一键换肤流程；System 会根据系统 UI 语言自动选择中文或英文（#351）。
