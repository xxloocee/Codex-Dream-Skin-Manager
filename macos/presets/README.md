...
- `image` 字段只能是**本目录内**的文件名（不能是路径），格式 `png` / `apng` / `jpg` / `jpeg` / `webp` / `gif` / `mp4`。图片 ≤ 128 MiB（建议 < 1 MiB），MP4 ≤ 128 MiB；MP4 必须为标准非分片 H.264/AVC 或 H.265/HEVC、最长 60 秒且最高 60 FPS，原文件直接保存并在 Codex 中静音循环播放，不会转码。
- 随发行版提供的内置预设必须声明 `appearance: "auto"`，让皮肤外壳跟随 Codex 客户端的浅暗设置；背景、遮罩与强调色需要在两种外观下都保持可读。`"dark"` / `"light"` 仍可用于用户自行创建或导入的单模主题，但不用于内置预设。
