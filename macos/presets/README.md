...
- `image` 字段只能是**本目录内**的文件名（不能是路径），格式 `png` / `apng` / `jpg` / `jpeg` / `webp` / `gif` / `mp4`。图片 ≤ 10 MiB（建议 < 1 MiB），MP4 ≤ 30 MiB；MP4 必须为标准非分片 H.264/AVC、最长 60 秒且最高 60 FPS，原文件直接保存并在 Codex 中静音循环播放，不会转码。
