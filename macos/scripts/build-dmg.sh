...
for runtime_script in apply-community-theme-macos.sh snapshot-active-theme-macos.sh \
  theme-switch-lock-macos.sh localization-macos.sh manager-actions-macos.sh; do
...
for manager_helper in delete-theme-macos.mjs import-batch-macos.mjs list-manager-themes-macos.mjs validate-image-macos.mjs; do
  [ -f "$MOUNTED_ENGINE/scripts/$manager_helper" ] \
    && [ ! -x "$MOUNTED_ENGINE/scripts/$manager_helper" ] \
    || { printf 'Mounted manager helper is missing or executable permissions are unsafe: %s\n' "$manager_helper" >&2; exit 1; }
done
[ -f "$MOUNTED_ENGINE/scripts/check-image-dimensions.mjs" ] \
  && [ ! -x "$MOUNTED_ENGINE/scripts/check-image-dimensions.mjs" ] \
  || { printf 'Mounted image preflight helper is missing or executable permissions are unsafe.\n' >&2; exit 1; }
