...
function validateTags(value) {
  let tags;
  try { tags = JSON.parse(value); } catch { throw new Error("tags-json must be a JSON array."); }
  if (!Array.isArray(tags) || tags.length > 8) throw new Error("tags-json must contain at most 8 tags.");
  return tags.map((tag, index) => {
    const normalized = validateText(String(tag), `tag ${index + 1}`, 20, "");
    if (!normalized) throw new Error(`tag ${index + 1} must not be empty.`);
    if (Array.from(normalized).length > 20) throw new Error(`tag ${index + 1} must be 20 characters or fewer.`);
    return normalized;
  });
}
...
if (!/\.(?:png|apng|jpe?g|webp|gif|mp4)$/i.test(image)) throw new Error("image must be a PNG, APNG, JPEG, WebP, GIF, or MP4 filename.");
...
const category = validateText(valueFor("category", "custom"), "category", 32, "custom");
const tags = validateTags(valueFor("tags-json", "[]"));
const requestedId = validateText(valueFor("theme-id", `custom-${Date.now()}`), "theme-id", 80, `custom-${Date.now()}`);
if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(requestedId)) throw new Error("theme-id contains invalid characters.");
...
const positionX = hasValue("position-x") ? validateRange(valueFor("position-x"), "position-x", -1, 1) : null;
const positionY = hasValue("position-y") ? validateRange(valueFor("position-y"), "position-y", -1, 1) : null;
const zoom = hasValue("zoom") ? validateRange(valueFor("zoom"), "zoom", 1, 2) : null;
const positionMode = validateChoice(valueFor("position-mode", "locked"), "position-mode", ["locked", "free"]);
const framing = hasValue("framing") ? validateChoice(valueFor("framing"), "framing", ["true", "false"]) === "true" : false;
const framingEnabled = framing || positionX !== null || positionY !== null || zoom !== null || hasValue("position-mode");
...
if (framingEnabled) {
  custom.art.positionX = positionX ?? 0;
  custom.art.positionY = positionY ?? 0;
  custom.art.zoom = zoom ?? 1;
  custom.art.positionMode = positionMode;
  custom.art.framingEnabled = true;
}
