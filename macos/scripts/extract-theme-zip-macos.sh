...
if [ -f "$SOURCE_ROOT/manifest.json" ]; then
  ...
  [ "$official_backgrounds" -eq 1 ] \
    || fail_extract "Official theme ZIP must contain exactly one background.webp, background.jpg, background.png, background.apng, background.gif, or background.mp4."
