...
"$NODE" "$ROOT/tests/bundled-node-runtime.test.mjs"
...
if /usr/bin/grep -F -q 'CODEX_EXPECTED_TEAM_ID' "$ROOT/scripts/common-macos.sh" ||
    [ "$(/usr/bin/grep -F -c -- '--test-requirement' "$ROOT/scripts/common-macos.sh")" -lt 2 ]; then
...
"$NODE" -e '
  ...
  if (value.currentVersion !== "v1.7.7" || value.latestVersion !== "v9.8.7") process.exit(1);
...
/usr/bin/env HOME="$TMP/seed-home" /bin/bash -c '
  ...
  for id in preset-ink-feather-glow preset-silver-glass-dream; do
    [ -s "$themes/$id/theme.json" ] || exit 1
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "1.7.7" ]' _ "$ROOT"
