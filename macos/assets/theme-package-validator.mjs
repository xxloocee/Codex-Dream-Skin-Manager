  ...
  if (theme.art !== undefined) {
    ...
    for (const key of ["positionX", "positionY"]) {
      if (art[key] !== undefined && (typeof art[key] !== "number" || !Number.isFinite(art[key]) || art[key] < -1 || art[key] > 1)) {
        fail(`theme.json.art.${key} must be between -1 and 1`);
      }
    }
    if (art.zoom !== undefined && (typeof art.zoom !== "number" || !Number.isFinite(art.zoom) || art.zoom < 1 || art.zoom > 2)) {
      fail("theme.json.art.zoom must be between 1 and 2");
    }
    if (art.positionMode !== undefined && !new Set(["locked", "free"]).has(art.positionMode)) {
      fail("theme.json.art.positionMode is unsupported");
    }
    if (art.framingEnabled !== undefined && typeof art.framingEnabled !== "boolean") {
      fail("theme.json.art.framingEnabled must be boolean");
    }
